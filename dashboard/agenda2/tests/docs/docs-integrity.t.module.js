    ...
    const
        ...
        testTags = (t, tag, ignoreList) => {
            ...
            if (missingTags.length > 0) {
                t.fail(`Classes below don't have "@${tag}" docs tag. To ignore add class name to "tests/index.js?459414" to "ignore/${tag}s" test config array\n`);
                missingTags.forEach(cls => t.fail(`'${cls}'`));
            }

            if (missingClassTypes.length > 0) {
                t.fail(`Features below don't have "@classType" docs tag. To ignore add class name to "tests/index.js?459414" to "ignore/${tag}s" test config array\n`);
                missingClassTypes.forEach(cls => t.fail(`'${cls}'`));
            }

            if (ignoreList.length > 0) {
                t.fail(`Redundant test ignore items in "tests/index.js?459414" in "ignore/${tag}s" test config array`);
