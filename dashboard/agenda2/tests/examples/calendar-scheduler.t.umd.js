...
StartTest(t => {
  ...
  t.it('EventMenu', async t => {
    ...
    await t.type(null, ' ');
    await t.waitFor(() => calendar.features.eventMenu.menu.containsFocus); // Invoke the Edit item

    await t.type(null, ' ');
    await t.waitFor(() => calendar.features.eventEdit.editor.containsFocus); // Edit worked

    await t.type(null, 'Free Vodka', null, null, null, true);
    await t.click('[data-ref="saveButton"]');
    await t.waitFor(() => !calendar.features.eventEdit.editor.isVisible); // Invoke editor using ENTER directly on the event
    ...
    await t.click('[data-ref="saveButton"]');
    await t.waitFor(() => !calendar.features.eventEdit.editor.isVisible); // Edit worked

    t.is(freeBeer.name, 'Free Beer');
    await t.type(null, ' ');
    await t.waitFor(() => calendar.features.eventMenu.menu.containsFocus); // Invoke the Duplicate Event item

    await t.type(null, '[DOWN][DOWN][ENTER]'); // EventMenu worked
