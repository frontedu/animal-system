...
StartTest(t => {
  ...
  t.it('sanity', async t => {
    ...
    t.is(Array.from(monthView.contentElement.querySelectorAll(`.${monthView.dayCellCls}`)).filter(isNotHideDisplay).length, 30); // These two complex methods should not be run by default.
  ...
  t.it('Prev/next should only gather events once', async t => {
    ...
    await t.click('[data-ref="prevButton"]'); // No events in September

    await t.waitFor(() => t.query('.b-cal-event-wrap').length === 0);
    await t.click('[data-ref="nextButton"]'); // back to October's event set

    await t.waitFor(() => t.query('.b-cal-event-wrap').length === eventCount); // The navigation only caused two event collection runs.
  ...
  t.it('autoRowHeight', async t => {
    ...
    await t.waitForAnimations(); // It's reduced the events per cell

    t.isLess(monthView.eventsPerCell, eventsPerCell); // But because we are autoRowHeight : true, the eventsPerCell is not consulted
    ...
    await t.waitForAnimations(); // Returned to initial state
  ...
  t.it('Week row sizing', async t => {
    ...
    t.is(flexedWeekStart, weekStartDate, 'Event received correct date'); // Week has shrunk back
  ...
  t.it('Prev/next when MonthView contains cell for new date', async t => {
    ...
    await t.waitFor(() => t.query('.b-cal-event-wrap').length === 0); // This the month we are on now.
